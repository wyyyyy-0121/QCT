...
用户主导研究方向：提出核心问题、研究直觉和候选思路，构建总体研究框架，确定模型目标、输入契约、评价重点与创新边界，并对路线取舍和论文中的最终学术表述负责。

Codex负责把研究思路转化为可证伪的方法规格和实验协议，完成代码实现、单元测试、smoke、结构检查、工作簿诊断、候选检查、批量实验、失败定位以及CSV/报告/Excel审计，并基于实验事实提出可供用户审查的改进方案。

双方共同复盘每轮证据：用户判断思路是否符合研究目标并决定框架调整，Codex检查实现、数据边界和结论是否由证据支持。Codex提出的方案属于研究辅助建议，不自动视为已成立的创新或论文结论。
