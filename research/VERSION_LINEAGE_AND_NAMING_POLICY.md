...
状态：2026-08-31更新。本文定义论文、演示和新公共接口使用的正式版本名。
...
2. 由新核心独立生成完整公式排名；若使用候选修复，只允许把它作为解释性探针，
   不能反向决定基础排名；
3. 只使用通过发现阶段跨结构信号门的证据，不预先强制公式、图、反事实等固定组合；
4. 不依靠固定“插入第3/第5名”覆盖V4结果；
5. 将全部既有揭晓数据只作开发，并使用候选锁后的旧试用包及全新保管人盲测分别
   完成冻结复现和最终确认。

复用V4最终基础排序的残差救援或选择性审查器只能属于V4系辅助模块。它即使在新盲测
上有效，也不能命名为V5或声称替代V4主核心。当前发现阶段与命名授权以
`V5_CORE_FIRST_PRINCIPLES_DISCOVERY_PLAN.md`为准。
