...
| `FCRL` | 冻结 ForTaP 骨干上的反事实后悔值定位研究 | 已预注册；尚未获取权重或实现 | 研究代号，不是正式版本 |
...
| FCRL U0-U4 | 已预注册，U0 尚未运行 | 等待基础 ForTaP 权重接收与适配器可复现审计 |
...
仍未接收并保持未见。FCRL 已在权重获取和实现前预注册，允许只使用冻结 ForTaP 基础
骨干与既有 SpreadsheetBench v1 input-only 语料实现反事实后悔值定位头；它通过 U0-U4
前不是 `V5-R1`，也不得读取保密数据。路径级偏差见
