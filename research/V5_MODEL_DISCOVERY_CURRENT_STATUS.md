...
| `FCRL` | 冻结 ForTaP 骨干上的反事实后悔值定位研究 | U0 适配/工件门通过；U1-U4 未运行 | 研究代号，不是正式版本 |
...
| FCRL U0-U4 | U0 通过，U1-U4 未运行 | 205/205 骨干张量匹配，泄漏/重复哈希/GPU 前后向通过；下一步构建 v1 input-only 训练样本 |
...
input-only 语料训练公式解码器。U1-U4 通过前它不是 `V5-R1`，也不得读取保密数据。
U0 结果见 `research/V5_FCRL_U0_RESULT.md`，路径级偏差见
