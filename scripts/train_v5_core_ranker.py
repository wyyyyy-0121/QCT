...
def template_fold(template_family: str) -> int:
    digest = hashlib.sha256(template_family.encode("utf-8")).digest()
...
def ranking_top_margin(ranking: list[dict], *, learned_config: dict | None = None) -> tuple[float, float]:
    if learned_config is None:
        ...
        scores = sorted((
            _learned_score(row["evidence"]["feature_vector"], learned_config)
            for row in ranking
        ), reverse=True)
...
def main() -> None:
    ...
    for instance_id, shard in shards.items():
        ...
        fold = template_fold(rows[instance_id]["template_family"])
        grouped_pairs[fold].extend((positive, negative) for negative in negatives)
