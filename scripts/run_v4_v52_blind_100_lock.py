...
def verify_precommit(
    ...
    public_precommit_path: Path,
    manifest_ids: set[str],
    expected_events: int,
) -> dict[str, object]:
    commitment = json.loads(commitment_path.read_text(encoding="utf-8"))
    if int(commitment.get("expected_total_events", -1)) != expected_events:
        raise ValueError("Precommit event count differs from expected-events")
    previous = set(commitment.get("previously_revealed_ids", []))
    new_blind = set(commitment.get("new_blind_ids", []))
    if previous & new_blind or previous | new_blind != manifest_ids:
        raise ValueError("Precommit cohort IDs do not partition the manifest exactly")
    public_text = public_precommit_path.read_text(encoding="utf-8-sig").lower()
    for key in (
        "labels_sha256", "exceptions_sha256", "private_batch2_archive_sha256"
    ):
        digest = str(commitment.get(key, "")).lower()
        if len(digest) != 64 or digest not in public_text:
            raise ValueError(f"Public secret precommit does not contain {key}")
    return commitment
...
def main() -> None:
    ...
    parser.add_argument("--public-precommit", type=Path, required=True)
    ...
    try:
        ...
        rows, workbook_hashes = validate_public_manifest(
            args.manifest, expected_events=args.expected_events
        )
        commitment = verify_precommit(
            args.commitment,
            args.public_precommit,
    ...
    print(
        f"[scheduler] independent_workbooks={len(tasks)} workers={workers}",
        flush=True,
    )
    ranking_rows = []
    decision_rows = []
    with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
        ...
        for index, future in enumerate(concurrent.futures.as_completed(futures), 1):
            ...
            print(
                f"[{index}/{len(tasks)}] {instance_id} :: v4 + v5.2-{variant}",
                flush=True,
            )
    ...
    source_paths = [
        ...
        root / "scripts/run_v4_v52_blind_100_lock.py",
        root / "scripts/score_v4_v52_blind_100.py",
    ...
    metadata = {
        ...
        "precommit": {
            "commitment_file": str(args.commitment.resolve()),
            "commitment_sha256": sha256_file(args.commitment),
            "public_precommit_file": str(args.public_precommit.resolve()),
            "public_precommit_sha256": sha256_file(args.public_precommit),
            "labels_sha256": commitment["labels_sha256"],
            "exceptions_sha256": commitment["exceptions_sha256"],
            "private_batch2_archive_sha256": commitment[
                "private_batch2_archive_sha256"
            ],
            "previously_revealed_event_count": len(
                commitment["previously_revealed_ids"]
            ),
            "new_blind_event_count": len(commitment["new_blind_ids"]),
        },
    }
