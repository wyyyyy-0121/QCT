"""Freeze model trees, parameters, scorer, and gates before confirmation data."""
...
def inspect_model(source: Path, kind: str) -> dict:
    if kind == "v4_r1":
        ...
        expression = (
            "import importlib,inspect,json;m=importlib.import_module('formulaguard.v5_1_development');"
            "print(json.dumps({'entry_point':'formulaguard.v5_1_development.v5_1_development_scores',"
            "'signature':str(inspect.signature(m.v5_1_development_scores)),"
            "'parameters':m.v5_1_development_default_parameters()}))"
        )
...
def main() -> int:
    ...
    artifact_paths = (
        ...
        "scripts/score_v51_natural_confirmation.py",
        "scripts/create_v51_reveal_authorization.py",
        "scripts/freeze_v51_natural_confirmation_models.py",
        "formulaguard/v5_1_development.py",
    )
    lock = {
        "protocol": "v51_natural_confirmation_model_lock_v1",
        "models_frozen_before_dataset_generation": True,
        "labels_read": [],
        "models": models,
        "artifacts": {path: sha256(ROOT / path) for path in artifact_paths},
        "dataset_protocol": {
            "release": "v51_natural_confirmation_release_v1",
            "cases": 240,
            "clusters": 20,
