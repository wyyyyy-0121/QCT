...
def main():
    ...
    lines = [
        ...
        "",
        "> 本报告中的数值来自原始 CSV；它们是当前代码和当前数据版本的测量结果，不代表竞赛获奖保证。",
        "",
        "## 1. 数据有效性",
        "",
    ...
    lines += [
        ...
        f"- FormulaGuard 平均 MRR 差：{comparison['mean_mrr_difference']:.4f}",
        f"- Bootstrap 95% CI：[{comparison['bootstrap_95_ci'][0]:.4f}, {comparison['bootstrap_95_ci'][1]:.4f}]",
        f"- FormulaGuard 更优比例：{pct(comparison['formula_guard_better_fraction'])}",
        "",
        "## 4. 干净工作簿与失败案例",
        "",
        ...
        f"- 校准干净表警报率：{pct(clean['calibration_clean_alarm_rate'])}",
        f"- 当前数据干净表警报率：{pct(clean['alarm_rate'])}",
        f"- 阈值验收通过：{clean['threshold_gate_passed']}",
        f"- FormulaGuard 落后于最强无真值基线的实例数：{len(failure_rows)}",
        "- 失败实例详见 `failure_cases.csv`。",
        "",
        "## 5. 写论文前必须检查",
        "",
        "1. 若置信区间跨过 0，不得写成稳定显著优于基线。",
        "2. 合成数据与真实错误数据必须分表报告。",
        "3. `-like` 基线不得冒充原作者完整实现。",
        "4. 检查 `by_depth.csv`、`by_error.csv` 和失败实例，不只看总体均值。",
        "5. 所有论文表格应由这些 CSV 重新生成，禁止手工修改数字。",
        "",
    ]
